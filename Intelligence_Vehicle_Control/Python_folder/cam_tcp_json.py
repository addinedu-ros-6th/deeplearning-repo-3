    ...
    try:
        while True:
            ...
            if frame1 is not None:
                client_socket.send(frame1.encode('utf-8'))
            else:
                client_socket.send(json.dumps({'type': 'image', 'index': 0, 'image': ''}).encode('utf-8'))
            ...
            if frame2 is not None:
                client_socket.send(frame2.encode('utf-8'))
            else:
                client_socket.send(json.dumps({'type': 'image', 'index': 1, 'image': ''}).encode('utf-8'))
